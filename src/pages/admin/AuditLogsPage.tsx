    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="flex flex-col gap-1">
          <label className="text-xs text-slate-500">Actor Email</label>
          <input
  ...
  {logs.map((log) => (
    <div key={log.id} className="p-4 space-y-3">
      ...
      <div className="flex justify-between items-start gap-3">
        ...
        <span
          className={`px-2 py-1 text-[10px] rounded-md font-medium whitespace-nowrap ${
      ...
      {/* Expanded Section */}
      {expandedId === log.id && (
        <div className="pt-3 space-y-3 text-xs border-t border-slate-100">
          <div>
            <span className="font-semibold text-slate-700">
              Old Value:
            </span>
            <pre className="mt-1 bg-slate-50 border rounded-md p-3 overflow-x-auto">
              {log.oldValue ?? "—"}
            </pre>
          </div>
